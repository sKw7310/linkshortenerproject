...
import { createLinkForUser, getLinkByShortCode, updateLinkForUser, deleteLinkForUser } from '@/data/links';
...
export async function createLink(data: unknown) {
  ...
  try {
    ...
    if (shortCode) {
      ...
      if (existingLink) {
        return { error: `Short code "${shortCode}" is already taken. Please choose another.` };
      }
    } else {
